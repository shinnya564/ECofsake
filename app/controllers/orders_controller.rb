...
class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.payment_flg == 'クレジットカード'
      ...
      begin
        ...
        redirect_to(request.referer, notice: 'このカードはご利用になれません。お手数ですが、窓口までお問い合わせください。') && (return)
      end
  ...
  def order_check
    ...
    if @payment_flg == '0'
      ...
      if card == ''
        redirect_to(request.referer, notice: 'クレジットカードを選択してください') && return
      else
        @card = Card.find(card)
        ...
        @secret = '**** **** **** ' + @default_card_information.last4.to_s
      end
